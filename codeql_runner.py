...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=".",
        type=Path,
        help="Source root for CodeQL extraction (default: current directory).",
    )
    parser.add_argument(
        "--db-out",
    ...
    parser.add_argument(
        "--sarif-out",
        default=Path("..") / "codeql-results.sarif",
        type=Path,
        help="Path to write the SARIF report (default: ../codeql-results.sarif).",
    )
    parser.add_argument(
        "--suite",
        default=None,
...
def main() -> None:
    args = parse_args()
    error_log_path = Path.cwd().parent / ERROR_LOG_NAME
